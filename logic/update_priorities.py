...
def run_update_priorities(base_url: str, api_key: str, source_shop_id: str, source_menu_id: str, dest_shop_id: str, dest_menu_id: str, dest_lang_id: str) -> Generator[str, None, None]:
    ...
    for item in source_menu_items:
        if item.get("lang_data"):
            name = item["lang_data"][0].get("name")
            priority = item["lang_data"][0].get("priority")
            if name:
                source_priority_map[name] = priority
    
    yield f"🗺️ Stworzono mapę priorytetów na podstawie {len(source_priority_map)} unikalnych nazw."
    ...
    for item in dest_menu_items:
        if item.get("lang_data"):
            dest_name = item["lang_data"][0].get("name")
            current_priority = item["lang_data"][0].get("priority") 
            dest_item_id = item.get("item_id")
            
            if dest_name in source_priority_map:
                source_priority = source_priority_map[dest_name]
                
                if current_priority != source_priority:
                    yield f"➡️ Znaleziono różnicę dla '{dest_name}': jest {current_priority}, powinno być {source_priority}. Przygotowuję aktualizację dla języka '{dest_lang_id}'."
                    update_item = {
